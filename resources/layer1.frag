...
void main(void) {
	...
	float step1 = u_resolution.y * 0.50;

	vec4 color1 = HSLToRGB(transformHSL(RGBToHSL(blue1), fract(u_time / 10.0), 1.0, 0.8));
	vec4 color2 = HSLToRGB(transformHSL(RGBToHSL(blue2), fract(u_time / 20.0), 1.0, 0.8));
	vec4 color3 = HSLToRGB(transformHSL(RGBToHSL(blue1), fract(u_time / 16.0), 1.0, 0.8));

	vec4 color = mix(color1, color2, smoothstep(u_resolution.y, step1, y));
	color = mix(color, color3, smoothstep(step1, 0.0, y));
